...
升级前退出游戏，把整个 `Ports/BalatroDual/saves/` 复制到电脑另行备份，
不要只复制 `save.jkr`；设置和各档位的解锁进度也在该目录内。
解压时合并目录，不要先删除整个 `BalatroDual`。若升级后存档异常，
退出游戏并保留异常现场，再用备份恢复整个 `saves/`。
...
R4.5 修正双屏 CRT 采样与噪点、计分火焰显示、手牌底板和消耗牌按钮位置，
并在保留减少动态效果设置的同时恢复计分卡牌的抖动动画。

R4.5 已完成隔离实机视觉检查、11 组 Lua 回归测试，以及正式入口启动、
主菜单截图和正常退出检查；安装后的 312 个生成文件与测试基线逐项校验一致。
本轮升级前已备份整个存档目录，升级前后当前牌局 `save.jkr` 校验一致，
原始游戏 EXE 未改动。公开包仅调整说明和发布元数据，补丁与运行时保持一致。
这些检查不等同于全部牌组、全部机制、长期稳定性或完整实体触控验收。
...
- [RGDSplus 双屏与触控移植手册](docs/RGDSPLUS_PORTING_GUIDE.zh-CN.md)：设备拓扑、
  固件与应用路径、双屏渲染、物理触摸、已知故障及验收边界。
- [下一款双屏游戏候选分析](docs/DUAL_SCREEN_CANDIDATES.zh-CN.md)：重点评估
  《杀戮尖塔》的现有 PortMaster 基础、双屏交互与分阶段验证方案。

```text
adapter/       发布用适配文件；无游戏本体
metadata/      文件校验清单、补丁来源映射
tools/         仅适配包构建与审计工具
docs/          安装排错、发布说明、移植经验与候选研究
```
